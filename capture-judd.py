...
output_video = cv2.VideoWriter(output_video_name, cv2.VideoWriter_fourcc(*'mp4v'), 16, (1280, 720))
   ...
   # display_frame = frame1
    ...
    for i in range(len(scores)):
        if scores[i] > min_conf_threshold:
            ...
            frame_detections.append([current_frame, int(scores[i] * 100), ymin, xmin, ymax, xmax])

    if not detections_found:
        frame_detections.append([current_frame, 0, 0, 0, 0, 0])

    detection_data.extend(frame_detections)

    current_frame += 1

    # Ajouter un bouton rouge dans la fenêtre
    cv2.rectangle(display_frame, (10, 10), (55, 30), (0, 0, 255), -1)
    cv2.putText(display_frame, 'stop', (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
with open(csv_file_path, 'a', newline='') as file:
    ...
    if start_frame_count == 0:
        writer.writerow(['frames', 'Confidence', 'Ymin', 'Xmin', 'Ymax', 'Xmax'])
